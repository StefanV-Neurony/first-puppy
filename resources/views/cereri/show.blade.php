@extends('layouts.app')
@section('content')
    <h1 class="chestionar-title">Chestionar adoptie animal</h1>
    ...
    <fieldset disabled>
    {{ Form::open(array('class' => 'form-adoptie form-description form-create-cerere'))}}
    ...
    <div class="cerere-show-actions container-create-cerere">
        @if($cerere->status == '1')
        <div class="form-group">
            <a class="btn btn-sm btn-outline-primary" href="{{action('CereriController@downloadPDF', $cerere->id)}}">Descarca contract PDF<span><i class="fa fa-download"></i></span>  </a>
        </div>
        @endif
        @if(auth()->user()->type == '0')
            {{ Form::model($cerere, array('route' => array('cereri.aproba', $cerere->id), 'method' => 'PUT')) }}
            <div class="form-group">
                @if($cerere->status != '1')
            <button class="btn btn-success">Aproba</button>
            </div>
            {{Form::close()}}
            {{ Form::open(array('url' => 'cereri/' . $cerere->id, 'class' => 'form-delete ')) }}
            {{ Form::hidden('_method', 'DELETE') }}
            {{ Form::submit('Refuza', array('class' => 'btn btn-danger')) }}
            {{ Form::close() }}
            @endif
            <a class="btn btn-sm btn-outline-primary" href="{{action('CereriController@downloadPDF', $cerere->id)}}">Descarca contract PDF<span><i class="fa fa-download"></i></span>  </a>
        @endif
